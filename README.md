...
| **Files** | Indicator: `REMA.2.5.22092026.0225.pine`<br>TradingView description: `REMA_TradingView_About.txt` |
...
> 💡 **When updating to a new release**, remove the old indicator from the chart and add the new one. TradingView remembers old settings, so new defaults (for example, Short signals off) only apply to a freshly added indicator.
...
| **Short** | Blue diamond ◆ at the top, "SHORT" | Short entry. Off by default. |
| **Cover** | Orange circle ● at the bottom, "COVER" | Short exit. Off by default. |
...
| **Short** | Blue diamond ◆ above the bar, no text | Short entry. Off by default. |
| **Cover** | Orange circle ● below the bar, no text | Short exit. Off by default. |
...
2. **Then entries.** If flat and the Bullish condition holds (and long signals are on) → **Buy**, go long. Otherwise, if flat and the Bearish condition holds (and short signals are on) → **Short**, go short.
...
| Show short signals (SHORT / COVER) | ⬜ Off | Allow short entries and their exits, in the pane and on the chart. |
...
> The signal toggles change **what trades are tracked**, not just what is drawn. With short signals off, the script stays flat after a Sell until the next Buy.
...
3. Turn on **Show short signals** only if you trade both directions.
...
| Short | `REMA ◆ Short on {{ticker}} at {{close}}` | A Short (short entry) prints, when short signals are on |
| Cover | `REMA ● Cover on {{ticker}} at {{close}}` | A Cover (short exit) prints, when short signals are on |

Each message carries the same symbol drawn on the chart, so an alert can be matched to its marker at a glance.

Alerts follow the **signal toggles** (long/short), because those decide which trades exist. They ignore **Also show markers on price chart**, which only affects drawing. `alertcondition()` uses the settings saved when the alert was created, so changing a toggle later does not affect a running alert.
...
- **No backtest statistics.** REMA is an indicator, not a strategy. It shows trade pairs but does not measure their profit, costs or winrate.
...
|---|---|---|
| 2.5.22092026.0225 | 22 Sep 2026 | Naming cleanup: header description and code comments reworded. Logic unchanged. README and TradingView description added. |
...
The **Pine v6 conversion**, the **trend conditions and position state machine**, the **Buy / Sell / Short / Cover markers**, the **price-chart markers**, the **transparent candles** and the **alert set** are the work of **Subhasom Mandal**.
